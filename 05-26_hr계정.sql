...
select distinct job from emp; -- all은 생략가능
SELECT
    * FROM
    emp order by job, sal DESC;
    
select ename 이름,  job 직책, sal 급여 -- as 생략가능
from emp
order by sal desc;

select name 지도교수이름, position 직책, hiredate 입사일, deptno 과번호 
from professor
